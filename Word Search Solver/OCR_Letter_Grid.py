...
def OCRLG(x):
    ...
    while done != 1:
        ...
        for string in listofstrings2: #iterate through the filtered output list
            ...
            splitlist = re.split("\s", string) #break the string into an array of strings for indexing purposes
            if int(lastrow) >= int(splitlist[0]) - 5 or int(lastrow) <= int(splitlist[0]) - 5:  # check to see if the vertical start position of the line has changed. The OCR measures in pixels, so the 
